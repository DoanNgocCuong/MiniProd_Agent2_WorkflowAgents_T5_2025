...
class Memory(MemoryBase):
    def __init__(self, config: MemoryConfig = MemoryConfig()):
        ...
        print(self.config.vector_store.config)
    ...
    def add(
        ...
        print(f"vector_store_result: {vector_store_result}")
    ...
    def _add_to_vector_store(self, messages, metadata, filters, infer):
        ...
        added_memories = []
        memories_operations = {}
        print(f"new_retrieved_facts: {new_retrieved_facts}")
        try:
            for resp in new_memories_with_actions.get("memory", []):
                logging.info(resp)
                try:
                    if resp.get("text") in new_retrieved_facts:
                        memories_operations[resp.get("text")] = {
                            "text": resp.get("text"),
                            "operation": resp.get("event"),
                        }
        ...
        returned_memories.append({
            "retrieved_facts": new_retrieved_facts,
            "memories_operations": memories_operations,
        })

        
        print(f"returned_memories: {returned_memories}")
        return returned_memories
...
class AsyncMemory(MemoryBase):
    ...
    async def _add_to_vector_store(self, messages, metadata, filters, infer):
        ...
        print(f"fact_results: {fact_results}")
        ...
        try:
            ...
            for resp in new_memories_with_actions.get("memory", []):
                ...
                try:
                    ...
                    logging.error(f"Error in new_memories_with_actions: {e}")
            print(f"memory_tasks: {memory_tasks}")
